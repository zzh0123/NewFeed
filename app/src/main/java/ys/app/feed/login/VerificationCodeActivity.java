...
public class VerificationCodeActivity extends Activity {
    ...
    private String url_login, code, phone; // 获取验证码接口url, 验证码, 密码, 手机号
    ...
    private void login(){
        ...
        Okhttp3Utils.postAsyRequest_Form(url_login, paramsMap_login, new ResultCallback() {
            ...
            public void onFailure(Exception e) {
                ToastUtils.showShort(VerificationCodeActivity.this, "服务器未响应！");
            }
...
//                    JSONObject jsonObject = JSON.parseObject(str_getCode);
                    if (resultInfo.getCode() == 800) {
                        ToastUtils.showShort(VerificationCodeActivity.this, "登录成功！");
                        ...
                        SPUtils.put(VerificationCodeActivity.this, "userId", userId);
                        SPUtils.put(VerificationCodeActivity.this, "isGiftPackage", isGiftPackage);
                        if (intention == 0){ // 0.已选择意向产品
                            Intent intent = new Intent(VerificationCodeActivity.this, MainActivity.class);
                            startActivity(intent);
                        } else {
                            Intent intent = new Intent(VerificationCodeActivity.this, IntentionActivity.class);
                            startActivity(intent);
                        }
                        finish();
                    } else {
                        ToastUtils.showShort(VerificationCodeActivity.this, resultInfo.getMessage());
                    }
                } else {
                    ToastUtils.showShort(VerificationCodeActivity.this, "服务器返回数据为空！");
                }
            }
        });
    }
