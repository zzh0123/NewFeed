...
public class Okhttp3Utils {
     ...
     * @param callback
     */
    public static void getAsycRequest(String url, HashMap<String, String> paramsMap, final ResultCallback callback) {
        build_get_url(url, paramsMap); // 拼接get请求参数,无参数时传null
        LogUtils.i("---url---" + build_get_url(url, paramsMap));
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
        //2.创建Request对象，设置一个url地址（例如百度地址）,设置请求方式。
        Request request = new Request.Builder().url(build_get_url(url, paramsMap))
                .method("GET", null)
                .build();
        //3.创建一个call对象,参数就是Request请求对象
        Call call = okHttpClient.newCall(request);
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                LogUtils.i("---response---" + string.toString());
     ...
     * @param callback
     */
    public static void postAsycRequest_Json(String url, JSONObject json_params, final ResultCallback callback) {
        LogUtils.i("---url---" + url);
        LogUtils.i("---str_params---" + json_params.toJSONString());
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
        //2.创建请求体
        RequestBody requestBody = build_postJsonString_RequestBody(json_params);
    ...
    public static void postAsycRequest_String(String url, String str_params, final ResultCallback callback) {
        LogUtils.i("---url---" + url);
        LogUtils.i("---str_params---" + str_params);
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
        //2.创建请求体
     ...
     * @param callback
     */
    public static void postAsyRequest_Form(String url, HashMap<String, String> paramsMap, final ResultCallback callback) {
        LogUtils.i("---url---" + url);
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
     ...
     * @param url
     * @param file_list
     * @param filekey_list 表单name值
     * @param callback
     */
    public static void upLoadFile(String url, List<File> file_list, List<String> filekey_list,
                                  final ResultCallback callback) {
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
        //2.通过RequestBody.create 创建requestBody对象
        RequestBody requestBody = build_postMultiFile_RequestBody(file_list, filekey_list);
        //3.创建Request对象，设置URL地址，将RequestBody作为post方法的参数传入
     ...
     * @param file_list
     * @param filekey_list 表单name值
     * @param paramsMap    表单键值对
     * @param callback
     */
    public static void postAsyRequest_FormFile(String url, HashMap<String, String> paramsMap,
                                               List<File> file_list, List<String> filekey_list, final ResultCallback callback) {
        //1.创建OkHttpClient对象
        OkHttpClient okHttpClient = getInstance().mOkHttpClient;
        //2.通过RequestBody.create 创建requestBody对象
        RequestBody requestBody = build_postFormFile_RequestBody(paramsMap, file_list, filekey_list);
        //3.创建Request对象，设置URL地址，将RequestBody作为post方法的参数传入
        Request request = new Request.Builder().url(url).post(requestBody).build();
        //4.创建一个call对象,参数就是Request请求对象
        Call call = okHttpClient.newCall(request);
...
//    }
     ...
     *
     * @param file_list
     * @param filekey_list 文件表单name值
     * @return
     */
    private static RequestBody build_postMultiFile_RequestBody(List<File> file_list, List<String> filekey_list) {
        ...
        if (file_list != null && file_list.size() > 0) {
            for (int i = 0; i < file_list.size(); i++) {
                ...
                builder.addFormDataPart(filekey_list.get(i), file_list.get(i).getName(),
     ...
     * @return
     */
    private static RequestBody build_postFormFile_RequestBody(HashMap<String, String> paramsMap,
                                                              List<File> file_list, List<String> filekey_list) {
        MultipartBody.Builder builder = new MultipartBody.Builder();
        //设置类型是表单
        builder.setType(MEDIA_TYPE_MULTIPART_FORM);
        for (String key : paramsMap.keySet()) {
    ...
    public static String build_get_url(String baseUrl, HashMap<String, String> paramsMap) {
        if (paramsMap == null || paramsMap.size() == 0) {
            return baseUrl;
        } else {
            StringBuilder url_temp = new StringBuilder(baseUrl);
            url_temp.append("?");
            String url = "";
            ...
            while (it.hasNext()) {
                ...
                url_temp.append(key + "=" + value + "&");
            }
            url = url_temp.toString().substring(0,url_temp.toString().length() - 1);
            return url;
        }
    }
}
