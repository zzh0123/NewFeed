 ...
 * 团购商品详情（web）
 */
public class GoodsDetailWebActivity extends FragmentActivity implements View.OnClickListener, GoodsWebFragment.CallBackValue {
    ...
    private String userId; // 当前用户，被邀请人userId
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_group_buy_goods_detail_web);
...
//        classType = getIntent().getStringExtra("classType");
        classType = "2";
        isFromWeb = getIntent().getBooleanExtra("isFromWeb", true);
        if (isFromWeb){
            getValueFromWeb();
        } else {
            commodityId = getIntent().getStringExtra("commodityId");
    ...
    private void getValueFromWeb() {
        ...
        if (Intent.ACTION_VIEW.equals(action)) {
            ...
            if (uri != null) {
                commodityId = uri.getQueryParameter("commodityId");
                userId = uri.getQueryParameter("userId");
                groupNo = uri.getQueryParameter("groupNo");
                LogUtils.i("web--commodityId", "--commodityId--" + commodityId);
                LogUtils.i("web--userId", "--userId--" + userId);
                LogUtils.i("web--groupNo", "--groupNo--" + groupNo);
            }
        }
    }
    ...
    private void initView() {
        ...
        fragment_list.add(goodsWebFragment);
